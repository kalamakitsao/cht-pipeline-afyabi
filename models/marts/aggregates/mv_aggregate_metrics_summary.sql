{{
  config(
    materialized = 'view',
    indexes = [
      {"columns": ["county_id", "sub_county_id", "community_unit_id", "chp_area_id", "period_id", "metric_id"], "unique": true},
...
INNER JOIN {{ ref('mv_location_hierarchy') }} lh ON lh.chp_area_id = f.location_id
INNER JOIN {{ ref('dim_period') }} dp ON dp.period_id = f.period_id
